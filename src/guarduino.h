...
#define SOFTWARE_VERSION "2023.11.12.1"
...
extern const char *readSwitchSensor(uint64_t pinReadings, baseSensor thisSensor);
extern const char *switchValueON(baseSensor thisSensor);
extern const char *switchValueOFF(baseSensor thisSensor);
...
extern uint64_t readSensors(uint64_t currentBits, baseSensor *sensors, size_t sensorsSize);
extern void mqttSensorSendDiscovery(uint64_t pinReadings);
extern void getDeviceName(char *destbuf, size_t destbufsize);
extern void getSensorName(char *destbuf, size_t destbufsize, baseSensor thisSensor);
extern char *getSensorStateTopic(baseSensor thisSensor);
